...
class WIQAEvalPrediction(NamedTuple):
    """
...
class Trainer:
    ...
    def evaluate(self, task_name: str, eval_dataset: Optional[Dataset] = None) -> Dict[str, float]:
        """
        ...
        if task_name == "wiqa":
            output = self.wiqa_prediction_loop(eval_dataloader, description="Evaluation")
        else:
    ...
    def detail_evaluate(self, eval_dataset: Optional[Dataset] = None) -> Dict[str, float]:
        """
        Run evaluation and returns metrics.
        ...
        output = self.detail_prediction_loop(eval_dataloader, description="Evaluation")
    ...
    def predict(self, task_name: str, test_dataset: Dataset) -> PredictionOutput:
        """
        ...
        if task_name == "wiqa":
            return self.wiqa_prediction_loop(test_dataloader, description="Prediction")
        else:
    ...
    def wiqa_prediction_loop(
        ...
        preds: torch.Tensor = None
        label_ids: torch.Tensor = None
        all_ques_types: torch.Tensor = None
        model.eval()
        ...
        samples_count = 0
        for inputs in tqdm(dataloader, desc=description, disable=disable_tqdm):
            loss, logits, labels, question_types = self.wiqa_prediction_step(model, inputs, prediction_loss_only)
            batch_size = inputs[list(inputs.keys())[0]].shape[0]
            samples_count += batch_size
            if loss is not None:
                eval_losses.append(loss * batch_size)
            ...
            if question_types is not None:
                all_ques_types = question_types if all_ques_types is None else torch.cat((all_ques_types, question_types), dim=0)
        ...
        if self.args.local_rank != -1:
            ...
            if label_ids is not None:
                label_ids = self.distributed_concat(label_ids, num_total_examples=self.num_examples(dataloader))
            if all_ques_types is not None:
                all_ques_types =  self.distributed_concat(all_ques_types, num_total_examples=self.num_examples(dataloader))
        elif is_torch_tpu_available():
            # tpu-comment: Get all predictions and labels from all worker shards of eval dataset
            if preds is not None:
                preds = xm.mesh_reduce("eval_preds", preds, torch.cat)
        ...
        if label_ids is not None:
            label_ids = label_ids.cpu().numpy()
        if all_ques_types is not None:
            all_ques_types = all_ques_types.cpu().numpy()

        if self.compute_metrics is not None and preds is not None and label_ids is not None:
            metrics = self.compute_metrics(WIQAEvalPrediction(predictions=preds,
                                                              label_ids=label_ids,
                                                              question_types=all_ques_types
                                                              ))
        else:
            metrics = {}
        if len(eval_losses) > 0:
    ...
    def wiqa_prediction_step(
        ...
        question_types = inputs.get("question_types")
        if question_types is not None:
            question_types = question_types.detach()
        return (loss, logits.detach(), labels, question_types)
